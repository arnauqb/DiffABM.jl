...
function sample_quarantine(sampler, n_agents, time, start_time, end_time, p_quarantine)
...
function sir_step(model_params, params, S, I, R, time)
    ...
    does_quarantine = sample_quarantine(
        params.discrete_sampler, n_agents, time, quarantine_start_time,
    ...
    prob_recover = 1.0 - exp(-gamma * delta_t)
    ...
    @inbounds for i in 1:n_agents
        ...
        # Update deltas
        delta_I[i] = S[i] * sample_bernoulli(params.discrete_sampler, prob_infect)
        delta_R[i] = I[i] * sample_bernoulli(params.discrete_sampler, prob_recover)
    end
    S = S - delta_I
    I = I + delta_I - delta_R
    R = R + delta_R
