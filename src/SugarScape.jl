export SugarScapeParams, TwoPeakBoard, RandomAgentInitializer, GeneratedAgentInitializer,
       ArgmaxMovingRule, VonNeumannNeighborhood, MooreNeighborhood
...
@functor RandomAgentInitializer (vision_distribution, metabolic_rate_distribution,
    max_age_distribution, wealth_distribution, position_distribution)
...
function iterate(vnn::VonNeumannNeighborhood, i, j, vision)
    ...
    for dx in (-vision):vision
        for dy in (-vision):vision
            if abs(dx) + abs(dy) <= vision && (dx != 0 || dy != 0)
                new_i, new_j = i + dx, j + dy
                if 1 <= new_i <= vnn.board_length && 1 <= new_j <= vnn.board_length
                    push!(ret, (new_i, new_j))
                end
            end
        end
    end
...
function compute_move(board, agent, rule::ArgmaxMovingRule, occupied)
    ...
    for position in iterate(rule.neighborhood, agent.x[1], agent.y[1], agent.vision[1])
        ...
        score = sugar * (1.0 - occupied[position[1], position[2]])
        push!(scores, score)
    end
...
function move!(board, agent, move_onehot, rule::ArgmaxMovingRule, occupied)
    ...
    occupied[agent.x[1], agent.y[1]] -= ignore_gradient(agent.alive[1] * sum(move_onehot))
    for (i, position) in enumerate(iterate(
        rule.neighborhood, agent.x[1], agent.y[1], agent.vision[1]))
        new_x += position[1] * move_onehot[i]
        new_y += position[2] * move_onehot[i]
        occupied[position[1], position[2]] += ignore_gradient(move_onehot[i] *
                                                              agent.alive[1])
    end
...
function consume!(board, agent, neighbourhood, move_onehot)
    ...
    for (i, position) in enumerate(iterate(
        neighbourhood, agent.x[1], agent.y[1], agent.vision[1]))
    ...
    agent.wealth[1] = agent.alive[1] * (agent.wealth[1] + (new_wealth - agent.wealth[1]) -
                       agent.metabolic_rate[1])
end

function check_death!(agent, occupied, average_wealth)
    #prob_dying = 1.0 - exp(-agent.wealth[1])
    #lives = differentiable_is_greater(PiecewiseSmoothing([-0.5, 0.5]), agent.wealth[1], 0.0)
	lives = differentiable_is_greater(GaussianSmoothing(average_wealth), agent.wealth[1], 0.0)
	#println("lives is: ", lives)
	#println("wealth is: ", agent.wealth[1])
	#println("lives is: ", lives)
    #lives = sample_bernoulli(SM(), [prob_dying])[1]
    #has_wealth = differentiable_is_greater(StraightThroughSmoothing(), agent.wealth[1], 0.0)
    ...
    occupied[agent.x[1], agent.y[1]] = ignore_gradient(agent.alive[1])
end
...
function regenerate_sugar!(board, sugar_regeneration_rate, max_sugar_capacities)
    for i in axes(board, 1)
        for j in axes(board, 2)
            board[i, j] = board[i, j] +
                          sugar_regeneration_rate[1] * max_sugar_capacities[i, j]
        end
    end
end
...
function abm_step!(board, agents, occupied, moving_rule::ArgmaxMovingRule,
	...
	average_wealth = 1.0 #ignore_gradient(sum([agent.wealth[1] for agent in agents]) / length(agents))^2
    for idx in order
        agent = agents[idx]
...
function abm_run(params::SugarScapeParams)
    diff_type = promote_type(
        typeof(params.sugar_regeneration_rate[1]), get_type(params.board_initializer))
