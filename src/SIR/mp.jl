...
function propagate_inf(graph, edge_type, beta::T, transmission::AbstractVector{Q}) where {T, Q}
    sg = edge_type_subgraph(graph, edge_type)
    betas = beta .* ones(T, graph.num_nodes[edge_type[3]])
    cumulative_trans = propagate(_message, sg, +, xi = betas, xj = transmission)
...
#function propagate_inf(graph, edge_type, beta,
#        transmission::Vector{T}) where {T <: StochasticAD.StochasticTriple}
#    StochasticAD.propagate(
#        (beta, transmission) -> propagate_inf(graph, edge_type, beta, transmission),
#        beta, transmission, keep_deltas=true)
#end