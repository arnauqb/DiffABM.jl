...
function propagate_infection(graph, policies, beta::T, transmission, time) where {T}
    ...
    if is_complete(graph)
        return beta .* sum(transmission) .* ones(length(transmission)) / (nv(graph) - 1)
    end
    ...
    n_neighbours = propagate((xi, xj, e) -> xj, graph, +, xj = aux, xi=aux) / 2.0
    n_neighbours = max.(1.0, n_neighbours)
    return beta .* cumulative_trans ./ n_neighbours
end

function compute_transmission(
        graph, sampler, policies, beta, S, I, time, delta_t)
